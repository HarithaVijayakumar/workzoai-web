...
function TranscriptMessageRow({ item }: { item: TranscriptItem }) {
  ...
  return (
    <div className="border-b border-white/10 px-6 py-4 last:border-b-0">
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-white/10 bg-[#0b1424]">
      <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
        ...
        {showToggle ? <button type="button" onClick={onToggle} className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-black text-blue-100 transition hover:bg-white/10 sm:hidden">{collapsed ? "Expand" : "Collapse"}</button> : null}
      </div>
