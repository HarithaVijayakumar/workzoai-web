...
function buildHumanPauseMs(analysis: AnswerAnalysis) {
  ...
  if (analysis.state === "engaged" || analysis.state === "interested") return 650;
  return 1050;
}
...
function selectBrowserVoice(recruiterId: string) {
  ...
  const wantsFemale =
    recruiterId === "friendly_hr" || recruiterId === "startup_recruiter";
...
function openAiVoiceIdForRecruiter(recruiterId: RecruiterId) {
  if (recruiterId === "friendly_hr") return "shimmer";
  if (recruiterId === "startup_recruiter") return "shimmer";
  if (recruiterId === "german_corporate") return "alloy";
  return "echo";
}
...
function buildConversationalRecruiterSpeech({
  ...
  if (isOpening) {
    ...
    if (/how are you/i.test(question)) {
      if (recruiterId === "startup_recruiter") {
        return firstName ? `Hi ${firstName}. Can you hear me clearly?` : "Hi. Can you hear me clearly?";
      }
      if (recruiterId === "friendly_hr") {
        return firstName ? `Hi ${firstName}, nice to meet you. How are you today?` : "Hi, nice to meet you. How are you today?";
      }
      if (recruiterId === "german_corporate") {
        return firstName ? `Hello ${firstName}. Before we begin, how are you today?` : "Hello. Before we begin, how are you today?";
      }
      return firstName ? `Hi ${firstName}. How are you today?` : "Hi. How are you today?";
    }
  ...
  const memoryCallback = weakness
    ? `I noticed this pattern earlier too: ${weakness.toLowerCase()} `
    : strength && trust >= 68
      ? `That connects with a stronger point you showed earlier: ${strength.toLowerCase()} `
      : "";
  ...
  const pressurePrefix =
    state === "losing_confidence"
      ...
      : state === "pressuring"
        ...
        : state === "skeptical"
          ? "Hmm. I’m not fully convinced yet. "
  ...
  return `${pressurePrefix}${memoryCallback}${spokenBridge} ${spokenQuestion}`
    .replace(/\s+/g, " ")
    .trim();
}
...
function analyzeAnswer(
  ...
  if (words.length < 8 && !isLikelyInterviewAnswer(clean)) {
    return {
      ...
      bridge: "Okay.",
      followUp: "Can you give me a bit more context so I can evaluate the answer properly?",
    };
  }
  ...
  if (words.length < 18 && isLikelyInterviewAnswer(clean)) {
    return {
      ...
      state: "pressuring",
      trustDelta: -9,
      caption: "Recruiter needs more detail",
      bridge: "I need more than that to evaluate you fairly.",
      followUp: "Give me one concrete example: what was the situation, what did you personally do, and what changed because of it?",
      weakness: "Answer was too short to evaluate.",
    };
  }
  ...
  if (words.length > 135) {
    return {
      ...
      state: "pressuring",
      trustDelta: -10,
      caption: "Answer losing clarity",
      bridge: "I’ll stop you there because the answer is starting to lose focus.",
      followUp:
        "Give me the result in one sentence first, then tell me your exact contribution.",
      weakness: "Answer became too long and lost focus.",
    };
  }
  ...
  if (!ownershipWords) {
    return {
      ...
      state: repeatedOwnershipIssue ? "losing_confidence" : "skeptical",
      trustDelta: repeatedOwnershipIssue ? -13 : -9,
      caption: "Recruiter cannot see ownership",
      bridge: repeatedOwnershipIssue
        ? "I noticed this earlier too — your ownership is still not clear."
        : "I’m not clear on what you personally owned.",
      followUp: "What exactly did you do yourself — not the team, not the company, but you?",
      weakness: "Ownership was unclear.",
    };
  }
  ...
  if (!hasNumber) {
    return {
      ...
      state: repeatedMetricsIssue ? "losing_confidence" : "skeptical",
      trustDelta: repeatedMetricsIssue ? -12 : -8,
      caption: "Waiting for measurable impact",
      bridge: repeatedMetricsIssue
        ? "I’m noticing the same issue again: I still don’t hear measurable impact."
        : "That gives me context, but I still don’t hear the impact.",
      followUp:
        "What changed because of your work — time saved, issues reduced, quality improved, or customer impact? Give me the closest number you can.",
      weakness: "Missing measurable impact.",
    };
  }
  ...
  if (vagueWords && words.length < 70) {
    return {
      ...
      state: "skeptical",
      trustDelta: -6,
      caption: "Answer still feels generic",
      bridge: "That still sounds a little too general for an interview answer.",
      followUp:
        "Can you give me the concrete situation, your action, and the outcome?",
      weakness: "Answer sounded generic.",
    };
  }
  ...
  if (hasNumber && ownershipWords) {
    ...
    return {
      ...
      bridge: "That is more convincing because you gave me ownership and impact.",
...
function extractSafeCvSignals(setup: WorkZoInterviewSetup) {
  ...
  if (lower.includes("sql")) signals.push("SQL exposure");
  if (lower.includes("python")) signals.push("Python exposure");
  if (lower.includes("excel") || lower.includes("spreadsheet")) {
    signals.push("spreadsheet or Excel experience");
  }
  if (lower.includes("ticket") || lower.includes("incident") || lower.includes("service desk")) {
    signals.push("ticketing or service workflow experience");
  }
  if (lower.includes("project")) signals.push("project experience");
  if (lower.includes("data") || lower.includes("analytics") || lower.includes("analysis")) {
    signals.push("data/analytics exposure");
  }
...
function isLikelyInterviewAnswer(answer: string) {
  ...
  return /\b(i|my|we|our|project|worked|built|handled|resolved|improved|created|managed|led|analyzed|implemented|customer|team|process|result|impact)\b/i.test(lower);
}
...
function buildJobContext(setup: WorkZoInterviewSetup) {
  ...
  return source.replace(/\s+/g, " ").slice(0, 3500);
}
...
function buildClarificationReply(
  ...
  if (lower.includes("role") || lower.includes("company") || lower.includes("job")) {
  ...
  if (lower.includes("resume") || lower.includes("cv") || lower.includes("job description") || lower.includes("jd")) {
    if (hasCv && hasJob) {
...
export default function InterviewPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      if (window.speechSynthesis.onvoiceschanged === warmVoices) {
  ...
  const speakRecruiter = useCallback(
    (text: string, afterSpeak?: () => void) => {
      ...
      const voices = window.speechSynthesis.getVoices();
      const needsFemaleVoice =
        recruiterId === "friendly_hr" || recruiterId === "startup_recruiter";
      const hasPreferredVoice = voices.some((voice) =>
        needsFemaleVoice ? isLikelyFemaleVoice(voice) : isLikelyMaleVoice(voice),
      );

      if (!voices.length || !hasPreferredVoice) {
        let didSpeak = false;
        const speakOnce = () => {
          if (didSpeak) return;
          didSpeak = true;
        ...
        window.setTimeout(speakOnce, needsFemaleVoice ? 1100 : 650);
        return;
      }
    ...
    (answer: string) => {
      ...
      setVoiceStatus(
        analysis.state === "pressuring" ||
          analysis.state === "losing_confidence"
          ? "Recruiter is interrupting..."
  ...
  const startStandardInterview = useCallback(async () => {
    ...
    try {
      console.info("WorkZo recruiter voice mapping", {
        ...
        openAiVoiceId: recruiterVoiceId,
        mode: "voice",
      });
    } catch {}
        ...
        <div className="relative h-full min-h-screen overflow-hidden bg-[#020617]">
          ...
          <div className="absolute inset-x-0 bottom-28 z-50 mx-auto max-w-xl rounded-[26px] border border-violet-300/15 bg-violet-500/10 p-5 text-center backdrop-blur-2xl">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-300">
              For the Product Hunt demo, start with Standard Interview first, then record Live Interview separately if Tavus is enabled.
            </p>
            <button
              type="button"
