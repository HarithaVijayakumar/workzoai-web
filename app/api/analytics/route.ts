...
export async function GET() {
  ...
  // Backward-compatible shape for older founder dashboard widgets.
  // New dashboard should read `summary`, but `stats/recentEvents` prevents crashes.
  const stats = {
    totalEvents: summary.totalEvents ?? 0,
    interviewStarts: summary.interviewsStarted ?? 0,
    interviewCompleted: summary.completedInterviews ?? 0,
    completionRate: summary.completionRate ?? 0,
    mobileUsers: (summary.devices?.mobile ?? 0) + (summary.devices?.tablet ?? 0),
    desktopUsers: summary.devices?.desktop ?? 0,
    recruiterCounts: summary.recruiters ?? {},
  };
